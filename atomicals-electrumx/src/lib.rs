...
impl Http for ElectrumX {
        ...
        R: DeserializeOwned,
    {
        ...
        headers.insert(USER_AGENT,"PostmanRuntime/7.37.0".parse().unwrap());

        for _ in self.max_retries.clone() {
            match self.client.get(u).headers(headers.clone()).query(&[("params",&serde_json::to_string(&params).unwrap())]).send().await {
...
impl ElectrumXBuilder {
    ...
    pub fn testnet() -> Self {
        Self::default()
            ...
            .base_uri(env::var("TEST_RPC").unwrap_or("https://eptestnet.atomicals.xyz/proxy".to_string()))
    }
...
impl Default for ElectrumXBuilder {
    fn default() -> Self {
        Self {
            ...
            base_uri: "https://ep.atomicalmarket.com/proxy".into(),
        }
    }
}
